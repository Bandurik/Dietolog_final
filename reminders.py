...
def start_scheduler():
    ...
    schedule.every().day.at("20:30").do(send_reminders_for_schedule, "7-00", "Не забудьте покушать Ужин!")
    ...
    schedule.every().day.at("20:14").do(send_reminders_for_schedule, "9-00", "Не забудьте выпить воды!")
    schedule.every().day.at("18:40").do(send_reminders_for_schedule, "9-00", "Не забудьте покушать Ужин!")

    # Ежедневное и еженедельное меню
    schedule.every().day.at("20:23").do(send_daily_menu)
...
# Проверка подписок каждый день
    schedule.every().day.at("20:10").do(check_and_update_subscriptions)
